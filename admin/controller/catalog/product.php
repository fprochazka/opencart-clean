...
class ControllerCatalogProduct extends Controller {
	 ...
	 * @return string
	 */
	private function getUrlFilters($change = array())
	{
		$url = '';
		...
		foreach ($preserve as $key) {
			if (isset($this->request->get[$key])) {
				$url .= "&$key=" . (isset($change[$key]) ? $change[$key] : $this->request->get[$key]);
			}
		}
  	...
  	private function getList() {
		...
		$url = $this->getUrlFilters(array(
			'order' => ($order == 'ASC' ? 'DESC' : 'ASC')
		));

		$this->load->model('catalog/category');
		$this->data['categories_url'] = $this->url->link('catalog/product/addcategory', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	public function addcategory()
	{
		if (isset($this->request->post['selected']) && $this->validateForm()) {
			...
			/** @var \ModelCatalogCategory $categoryModel */
			/** @var \ModelLocalisationLanguage $langModel */
			$productModel = $this->load->model('catalog/product');
			$categoryModel = $this->load->model('catalog/category');
			$langModel = $this->load->model('localisation/language');

			$categoryDesc = $categoryModel->getCategoryDescriptions($category_id, $langId = $langModel->getCodeId('cs'));
			$categoryName = $categoryDesc['name'];

			foreach ($products as $productId) {
				if (!$productModel->addProductCategory($productId, $category_id)) {
					$productDesc = $productModel->getProductDescriptions($productId, $langId);
					$this->error['warning'][] = "Zboží {$productDesc['name']} už je v kategorii $categoryName";

				} elseif ($i = array_search($productId, $this->request->post['selected'], TRUE)) {
					unset($this->request->post['selected'][$i]);
				}
			}
			...
			if (empty($this->error['warning'])) {
				$this->session->data['success'] = "Žboží bylo přidány do kategorie $categoryName";
