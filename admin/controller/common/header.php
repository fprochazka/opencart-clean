...
class ControllerCommonHeader extends Controller {
	protected function index() {
		...
		$this->data['text_productNoCategory'] = $this->language->get('text_productNoCategory');
		$this->data['text_productImport'] = $this->language->get('text_productImport');
		...
		if (!$this->user->isLogged() || !isset($this->request->get['token']) || !isset($this->session->data['token']) || ($this->request->get['token'] != $this->session->data['token'])) {
			...
			$this->data['productNoCategory'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&filter_category=0', 'SSL');
			$this->data['productImport'] = $this->url->link('module/import', 'token=' . $this->session->data['token'], 'SSL');
