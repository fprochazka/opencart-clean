...
class ModelCatalogManufacturer extends Model
{
	...
	public function getManufacturers($data = array()) {
		if ($data) {
			...
			if (!empty($data['filter_name'])) {
				$where[] = " LCASE(name) LIKE '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "%'";
			}
	...
	public function getNameId($name)
	{
		...
		$manufacturers = $this->getManufacturers(array(
			'filter_name' => $name
		));

		if ($manufacturers) {
			$first = reset($manufacturers);
			$this->nameIds[$name] = $first['manufacturer_id'];

		} else {
			throw new Nette\InvalidStateException("Manufacturer name '$name' not found.");
		}
