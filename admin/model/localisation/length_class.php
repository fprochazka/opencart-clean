...
class ModelLocalisationLengthClass extends Model
{
	...
	public function getLengthClasses($data = array()) {
		if ($data) {
			...
			if (!empty($data['filter_unit'])) {
				$where[] = "lcd.`unit` = '" . $this->db->escape($data['filter_unit']) . "'";
			}
			...
			if (!$length_class_data = $this->cache->get('length_class.' . (int)$this->config->get('config_language_id'))) {
	...
	{
		if (isset($this->lengthUnits[$unit])) {
			return $this->lengthUnits[$unit];
		}

		$lengths = $this->getLengthClasses(array(
			'filter_unit' => $unit
		));

		if ($lengths) {
			$first = reset($lengths);
			$this->lengthUnits[$unit] = $first['length_class_id'];

		} else {
			throw new Nette\InvalidStateException("Length unit '$unit' not found.");
		}
