...
class ModelLocalisationLanguage extends Model
{
	...
	public function getLanguages($data = array()) {
		if ($data) {
			...
			if (!empty($data['filter_code'])) {
				$where[] = "`code` = '" . $this->db->escape($data['filter_code']) . "'";
			}
	...
	{
		if (isset($this->langCodes[$code])) {
			return $this->langCodes[$code];
		}
		...
		if ($languages) {
			...
			$this->langCodes[$code] = $first['language_id'];

		} else {
			throw new Nette\InvalidStateException("Language code '$code' not found.");
		}
