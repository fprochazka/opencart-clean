...
class ModelLocalisationWeightClass extends Model
{
	...
	public function getWeightClasses($data = array()) {
		if ($data) {
			...
			if (!empty($data['filter_unit'])) {
				$where[] = "wcd.`unit` = '" . $this->db->escape($data['filter_unit']) . "'";
			}
	...
	{
		if (isset($this->weightUnits[$unit])) {
			return $this->weightUnits[$unit];
		}

		$weights = $this->getWeightClasses(array(
			'filter_unit' => $unit
		));

		if ($weights) {
			$first = reset($weights);
			$this->weightUnits[$unit] = $first['weight_class_id'];

		} else {
			throw new Nette\InvalidStateException("Weight unit '$unit' not found.");
		}
