...
class ModelLocalisationReturnReason extends Model {
	...
	public function getReturnReasons($data = array()) {
      	if ($data) {
			...
			$cacheKey = 'localisation.return_reason.search.' . (int)$this->config->get('config_language_id');
			if (!$return_reason_data = $this->cache->get($cacheKey)) {
				...
				$this->cache->set($cacheKey, $return_reason_data);
			}
