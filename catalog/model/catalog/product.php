...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		$cacheKey = 'catalog.product.search.' . (int)$this->config->get('config_language_id') . '.' . (int)$customer_group_id . '.' . md5(serialize($data));
		if (!$product_data = $this->cache->get($cacheKey)) {
			...
			$this->cache->set($cacheKey, $product_data);
		}
	...
	public function getLatestProducts($limit) {
		...
		$cacheKey = 'catalog.product.latest.' . (int)$this->config->get('config_language_id') . '.' . $customer_group_id . '.' . (int)$limit;
		if (!$product_data = $this->cache->get($cacheKey)) {
	...
	public function getBestSellerProducts($limit) {
		...
		$cacheKey = 'catalog.product.bestseller.' . (int)$this->config->get('config_language_id') . '.' . $customer_group_id . '.' . (int)$limit;
		if (!$product_data = $this->cache->get($cacheKey)) {
	...
	{
		$cacheKey = 'catalog.product.search.' . md5(serialize($data));
		if (!$product_data = $this->cache->get($cacheKey)) {
