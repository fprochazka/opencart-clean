...
class ModelCheckoutVoucherTheme extends Model {
	...
	public function getVoucherThemes($data = array()) {
      	if ($data) {
			...
			$cacheKey = 'checkout.voucher_theme.' . (int)$this->config->get('config_language_id');
			if (!$voucher_theme_data = $this->cache->get($cacheKey)) {
				...
				$this->cache->set($cacheKey, $voucher_theme_data);
			}
